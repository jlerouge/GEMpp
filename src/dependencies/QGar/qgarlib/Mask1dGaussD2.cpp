...
/**
 * @file   Mask1dGaussD2.cpp
 * @brief  Implementation of class qgar::Mask1dGaussD2.
 *
 * See file Mask1dGaussD2.h for the interface.
 *
 * @author <a href="mailto:qgar-develop@loria.fr?subject=Qgar fwd Karl Tombre">Karl Tombre</a>
 * @date   Jul 3, 2001  10:30
 * @since  qgar 1.0
 */
...
{
  double r = aX / aSigma;
  return -aX * Math::QG_1_SQRT_2PI / pow(aSigma, 3.0) * exp(-0.5 * r * r);
}
...
Mask1dGaussD2::Mask1dGaussD2(const Mask1dGaussD2& aMask)
...
Mask1dGaussD2::Mask1dGaussD2(double aSigma)
{
  // Error < 0.001 on each side
  double limit = ceil(3.82922419517181 * aSigma);
  ...
  for (int idx = -n + 1 ; idx <= (n - 1) ; ++idx)
    {
      *pm++ =   MASK1DGAUSSD2_phi2( 0.5 - idx, aSigma)
              - MASK1DGAUSSD2_phi2(-0.5 - idx, aSigma);
    }

  // Fill extremities
  *pCoeffMap_ = - MASK1DGAUSSD2_phi2(n - 0.5, aSigma);  // -n
  *pm         =   MASK1DGAUSSD2_phi2(0.5 - n, aSigma);  // +n
}
...
Mask1dGaussD2& Mask1dGaussD2::operator=(const Mask1dGaussD2& aMask)
{
