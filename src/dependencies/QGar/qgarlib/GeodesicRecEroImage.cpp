...
/**
 * @file GeodesicRecEroImage.cpp
 *
 * @brief Implementation of class qgar::GeodesicRecEroImage.
 *
 * See file GeodesicRecEroImage.h for the interface.
 *
 *
...
GeodesicRecEroImage::GeodesicRecEroImage(GreyLevelImage& aMarkImg,
      ...
      || (aMarkImg.height() != aResImg.height()))
    {
	 ...
	 << ')';
      throw QgarErrorDomain(__FILE__, __LINE__,
			    "qgar::GeodesicRecEroImage::GeodesicRecEroImage(qgar::GreyLevelImage&, qgar::GreyLevelImage&)",
...
void GeodesicRecEroImage::perform(GreyLevelImage* aMarkImg,
				  GreyLevelImage* aResImg)
{
  ...
  GreyLevelImage::value_type minimum; // local minimum
  ...
  for (int i = 0; i < height; ++i)
	...
	{
	  // point-wise maximum between min(p U Ng+(p)) and Ip
	  minimum = *Jp;
  ...
  for (int i = 0; i < height; ++i)
    {
      for (int j = 0; j < width; ++j, --Jp, --Ip)
	{
	  // point-wise minimum between min(p U Ng-(p)) and Ip
	  minimum = *Jp;
  ...
  //**** APPARENTLY NOT USED!
  //**** GreyLevelImage::value_type *minIndice = markerPixMap;
  //**** GreyLevelImage::value_type *maxIndice = minIndice + width*height - 1;
