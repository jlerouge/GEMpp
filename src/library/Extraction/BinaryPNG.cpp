...
BinaryImage BinaryPNG::read(QString filename, uint borderWidth, QGEbw color) {
    if(!filename.endsWith(".png", Qt::CaseInsensitive))
        Exception(QString("The given input (%1) is not a *.png image.").arg(filename));
    if(!QFileInfo(filename).exists())
        Exception(QString("The given input (%1) does not exist.").arg(filename));
    QImage source = QImage(filename);
    if(source.format() != QImage::Format_Mono)
        Exception(QString("The given input (%1) is not a monochrome image.").arg(filename));
...
void BinaryPNG::write(BinaryImage &img, QString filename, uint borderWidth, QGEbw color) {
    if(!filename.endsWith(".png", Qt::CaseInsensitive))
        Exception(QString("The given output (%1) is not a *.png image.").arg(filename));
