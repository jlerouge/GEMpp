...
void GLPK::prepare() {
    ...
    if(qp_)
        GEM_exception("GLPK cannot solve quadratic programs, please use Gurobi or Cplex instead or switch to linear formulation.");
    ...
    config_.mip_gap = pow(10, -PRECISION);
...
double GLPK::solve(Solution *sol) {
    if(!model_)
        GEM_exception("GLPK solver must be initialized before solving.");
    double obj = 0;
    switch(lp_->getSense()) {
