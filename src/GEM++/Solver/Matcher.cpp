...
void Matcher::run() {
    obj_ = INFINITY;
    if(!pb_)
        GEM_exception("The matcher needs a problem to solve before running the matching algorithm.");
    if(!cfg_)
        GEM_exception("The matcher needs a configuration to run the matching algorithm.");
    if(!s_)
        GEM_exception("The matcher has failed initializing a solver, thus it can't run the matching algorithm.");
    ...
    if(f) {
        ...
        if(!cfg_->solution.isEmpty()) {
            ...
            if(!QString::compare(filename, GEM_AUTO))
