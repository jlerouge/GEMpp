...
Graph::Type Graph::fromName(QString name) {
    for(Type t = (Type)0; t < COUNT; t = (Type)((int)t + 1))
        if(QString(typeName[t]).startsWith(name, Qt::CaseInsensitive))
            return t;
    GEM_exception(QString("Graph type '%1' not recognized, please use d(irected) or u(ndirected).").arg(name));
    return COUNT;
}
...
void Graph::removeVertex(Vertex *v) {
    ...
    if(vertices_[index] != v)
        GEM_exception(QString("Index mismatch in vertices of the graph %1").arg(id_));
...
void Graph::removeEdge(Edge *e) {
    ...
    if(edges_[index] != e)
        GEM_exception(QString("Index mismatch in edges of the graph %1").arg(id_));
...
void Graph::fromFile(const QString &filename) {
    if(filename.endsWith(".gml", Qt::CaseInsensitive))
        ...
        GEM_exception(QString("%1 is not a *.gml, *.gxl nor *.xml file.").arg(filename));
...
void Graph::fromGXL(const QString &filename) {
    ...
    if(!file.open(QFile::ReadOnly | QFile::Text))
        GEM_exception(QString("Error while loading %1").arg(filename, file.errorString()));
